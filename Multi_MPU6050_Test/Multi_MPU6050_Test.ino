...
//-----------------------------------------mpu4------------------------------------------------
// MPU control/status vars
bool dmpReady4 = false;  // set true if DMP init was successful
uint8_t mpuIntStatus4;   // holds actual interrupt status byte from MPU
uint8_t devStatus4;      // return status after each device operation (0 = success, !0 = error)
uint16_t packetSize4;    // expected DMP packet size (default is 42 bytes)
uint16_t fifoCount4;     // count of all bytes currently in FIFO
uint8_t fifoBuffer4[64]; // FIFO storage buffer
//-----------------------------------------------------------------------------------------
// MPU control/status vars
bool dmpReady5 = false;  // set true if DMP init was successful
uint8_t mpuIntStatus5;   // holds actual interrupt status byte from MPU
uint8_t devStatus5;      // return status after each device operation (0 = success, !0 = error)
uint16_t packetSize5;    // expected DMP packet size (default is 42 bytes)
uint16_t fifoCount5;     // count of all bytes currently in FIFO
uint8_t fifoBuffer5[64]; // FIFO storage buffer
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
    ...
    mpu.setZAccelOffset(1788); // HIGH8 factory default for my test chip
